  ...
  const cards = filtered
    .map((m) => {
      ...
      <div class="machine-card-header">
        ...
        ${alertS.length ? `<div style="background:var(--amber-dim);border:1px solid var(--amber-bdr);border-radius:8px;padding:6px;flex-shrink:0;color:var(--amber)">${I.alert.replace("11", "13")}</div>` : ""}
      </div>
  ...
  document.getElementById("view_container").innerHTML =
    ...
    <div class="panel-card">
      <div class="table-responsive-wrapper"><table>
        ...
        <tbody>${rows || `<tr><td colspan="8" class="table-empty-state">${I.radio.replace("15", "32")}<br><br>Nenhum sensor encontrado</td></tr>`}</tbody>
      </table></div>
    </div>
  </div>`;
}
  ...
  document.getElementById("view_container").innerHTML =
    ...
    <div class="panel-card">
      <div class="table-responsive-wrapper"><table>
        ...
        <tbody>${rows || `<tr><td colspan="9" class="table-empty-state">${I.clipboard.replace("15", "32")}<br><br>Nenhuma ordem encontrada</td></tr>`}</tbody>
      </table></div>
    </div>
  </div>`;
}
  ...
  document.getElementById("view_container").innerHTML =
    ...
    <div class="panel-card">
      <table>
        ...
        <tbody>${rows || `<tr><td colspan="7" class="table-empty-state">${I.users.replace("15", "32")}<br><br>Nenhum usuário encontrado</td></tr>`}</tbody>
      </table>
    </div>
  </div>`;
}
  ...
  sensors: {
    ...
    save() {
      ...
      const data = {
        ...
        location: document.getElementById("f-location").value,
        minThreshold: Number(document.getElementById("f-min").value),
        maxThreshold: Number(document.getElementById("f-max").value),
        currentValue: Number(document.getElementById("f-val").value),
        status: document.getElementById("f-status").value,
  ...
  orders: {
    ...
    save() {
      ...
      const data = {
        ...
        estimatedHours: Number(document.getElementById("f-hours").value),
